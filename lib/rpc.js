...
// callers in app.js, lib/explorer.js and routes/index.js
...
var METHODS = {
  ...
  getmovementspg:         [{names: ['page'], type: 'int'},
                           {names: ['perpage'], type: 'int'},
                           {names: ['ordering'], type: 'bool'},
                           {names: ['mincoins'], type: 'int'},
                           {names: ['color'], type: 'int'}]
};
...
// to dominate the run.
//
// The idle timeout is not optional. breakoutd serves each accepted connection
// from a thread that sits blocked in ReadHTTP() and only notices fShutdown
// when it comes back round the loop -- which it does only when another request
// arrives or the peer hangs up. Closing the RPC acceptor at shutdown does not
// wake it. StopNode() then waits twenty seconds, prints "ThreadsRPCServer
// still running", and tears the process down regardless; the next request on a
// surviving connection locks an already-destroyed mutex and the daemon aborts
// on a Boost assertion.
//
// Bounding idle sockets keeps that window small. Keep-alive still does its job
// during a reindex, where calls are continuous and nothing goes idle; between
// one-a-minute syncs the sockets are simply closed, which is what the daemon
// wants to see.
var agent = new http.Agent({
  ...
  maxSockets: 8,
  maxFreeSockets: 4,
  timeout: 10000        // close a connection idle for 10s
});
