        ...
        if (block) {
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              ...
              // Address totals are no longer accumulated here. Balances, holder
              // counts and supply are read from the daemon's Explore index,
              // which is the chain's own arithmetic rather than a running sum
              // this indexer maintained and could drift from. Indexing a
              // transaction is now just storing the transaction.
              saveTxRecord();
            });
          });
        } else {
