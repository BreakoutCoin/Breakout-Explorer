...
function update_address(hash, txid, amount, currency, type, txtype, cb) {
  ...
  find_address(hash, function(address) {
    if (address) {
      ...
      if ( address.startsWith('coinbase-') ||
           address.startsWith('burnt-') || 
           address.startsWith('scavenged-') ) {
        ...
        if (block) {
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              var txtype = tx.flags
              // - fee = inputs - outputs
              // - fees for coinstake transactions will be 0, because
              //   the Breakout protocol enforces inputs to equal outputs
              // - fees for coinbase is the negated sum of all coinbase and
              //      scavenged fees
              // - fees for any other transaction will be positive because
              //   inputs (positive) will always exceed outputs (negative)
              // - burnt coins are not fees, example:
              //     inputs = 25, outputs = burnt = 24 ==> fees = 1
              var fees = {};
              lib.syncLoop(nvin.length, function (loopVin) {
                var i = loopVin.iteration();
                // values of coinbase in nvin are virtual, so not
                //    included here
                if (tx.flags != 'coinbase') {
                  if (nvin[i].currency in fees) {
                    fees[nvin[i].currency] += nvin[i].amount;
                  } else {
                    fees[nvin[i].currency] = nvin[i].amount;
                  }
                }
                update_address(nvin[i].address, txid, nvin[i].amount,
                               txtype, 'vin', function() {
                  loopVin.next();
                });
              }, function() {
                var burnt = {};
                lib.syncLoop(vout.length, function (loopVout) {
                  var j = loopVout.iteration();
                  if (vout[j].address) {
                    if (vout[j].address.startsWith("burnt-") {
                      if (vout[j].currency in burnt) {
                        burnt[vout[j].currency] += vout[j].amount;
                      } else {
                        burnt[vout[j].currency] = vout[j].amount;
                      }
                    }
                    if (vout[j].currency in fees) {
                      fees[vout[j].currency] -= vout[j].amount;
                    } else {
                      fees[vout[j].currency] = -vout[j].amount;
                    }

//                     if (currs.isDeck(vout[j].currency))
//                     {
// asdfjkl;
//                         update_deck(vout[j].currency,
//                                     vout[j].address,
//                                     vout[j].txid);
//                    }

                    update_address(vout[j].address, txid, vout[j].amount,
                                   txtype, 'vout', function() {
                      loopVout.next();
                    });
                  } else {
                    loopVout.next();
                  ...
                  // totals_out is the total of the outputs.
                  // Since fees = inputs - outputs, then
                  //    inputs = fees + outputs = fees + totals
                  // Coinbase example:
                  //    fees = -11 (negated coinbase + scavenged fees)
                  //    totals = 11
                  //    inputs = -11 + 11 = 0
      ...
      lib.get_blockhash(start + x, function(blockhash){
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            if (block && (typeof block.tx != 'undefined') && (typeof block.tx.length != 'undefined')) {
