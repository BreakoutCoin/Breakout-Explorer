...
// unique-address count, refreshed at most once a minute (see get_address_count)
var address_count_cache = {n: null, at: 0};
...
module.exports = {
  ...
  get_supply: function(cb) {
    if ( settings.supply == 'HEAVY' ) {
      ...
      // was: sum of every Address.balance in Mongo
      module.exports.get_moneysupply(settings.symbol, cb);
    } else if (settings.supply == 'TXOUTSET') {
      rpc.call('gettxoutsetinfo', function (body) {
        return cb(body.total_amount);
      });
    } else {
      // was: coinbase sent-minus-received from the synthetic Address row
      module.exports.get_moneysupply(settings.symbol, cb);
    }
  },
  ...
  // Unique addresses holding value, across every currency.
  //
  // Breakout encodes the currency in the address prefix, so an address belongs
  // to exactly one colour and summing getrichlistsize over the colours counts
  // each address once. It counts addresses holding more than a cent, which is
  // the daemon's own tracking floor -- dust below that is not indexed.
  //
  // Cached for a minute: it moves slowly and costs one call per currency.
  get_address_count: function(cb) {
    var now = Date.now();
    if (address_count_cache.n !== null && (now - address_count_cache.at) < 60000) {
      return cb(address_count_cache.n);
    }
    var colors = currs.counted(), total = 0, i = 0;
    (function next(){
      if (i >= colors.length) {
        address_count_cache = {n: total, at: now};
        return cb(total);
      }
      rpc.call('getrichlistsize', {color: colors[i]}, function(n){
  ...
  // Assemble the explorer's view of a transaction from the daemon's.
  //
  // This is the shape the tx index stores and /ext/gettx serves: deduplicated,
  // currency-tagged vin/vout in toshis, plus the fee, burn and volume figures
  // derived from them. Factored out of database.js's save_tx so that the
  // indexer and the endpoint cannot drift apart -- serving a transaction that
  // has not been indexed must produce the same answer as serving one that has.
  //
  // tx    - getrawtransaction(txid, 1) or gettransaction(txid)
  // block - the confirming block, for its height; null for an unconfirmed tx
  assemble_tx: function(tx, block, cb) {
    module.exports.prepare_vin(tx, function(nvin) {
      module.exports.prepare_vout(tx.vout, tx.txid, nvin, function(vout, nvin2) {
        var vin = nvin2 || nvin;

        // fee = inputs - outputs, with coinbase's virtual inputs excluded and
        // coinstake zero by protocol. Burnt outputs are not fees.
        var fees = {};
        ...
        vout.forEach(function(v) {
          if (!v.address) return;
          if (v.address.indexOf('burnt-') === 0) {
            burnt[v.currency] = (burnt[v.currency] || 0) + v.amount;
          }
          fees[v.currency] = (fees[v.currency] || 0) - v.amount;
        });

        module.exports.calculate_totals(vout, tx.flags, function(totals) {
          var total = 0;
          for (var i = 0; i < vout.length; i++) {
            if (vout[i].currency == settings.symbol) total += vout[i].amount;
          }
          return cb({
            txid: tx.txid,
            vin: vin,
            ...
            blockindex: (block && block.height != null) ? block.height : -1
          });
        });
      });
    });
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    // The daemon resolves prevouts itself: both getrawtransaction (verbose) and
    // gettransaction return value, currency and addresses on each vin entry.
    // Use them.
    //
    // This used to fetch the previous transaction for every input, which on
    // this chain is not a small thing -- a coinstake routinely has a couple of
    // hundred inputs, so indexing one cost a couple of hundred extra RPC round
    // trips purely to learn what the daemon had already told us.
    if (input.addresses && input.addresses.length &&
        input.value !== undefined && input.value !== null) {
      addresses.push({hash: input.addresses[0],
                      amount: input.value,
                      currency: input.currency});
      return cb(addresses);
    }

    // Fallback: a daemon that does not annotate vin. Resolve it the long way.
