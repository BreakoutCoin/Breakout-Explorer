...
module.exports = {
  ...
  // Colours worth counting addresses for: the currencies, not the Deck.
  // A card's supply is 1 and its holders are tracked by getcardinfo, so
  // including them would add 53 to a count of addresses holding value.
  counted : function() {
    return Object.keys(ordering)
                 .filter(function(t){ return t.charAt(0) !== 'D'; })
                 .map(function(t){ return ordering[t]; });
  },
